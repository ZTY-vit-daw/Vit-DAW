...
$runDir = Join-Path $RepoRoot ("artifacts\static_eq_external_holdout\" + $stamp)
...
$before = [ordered]@{}
foreach ($path in $protectedFiles) { $before[$path] = Get-HashOrEmpty $path }
$envOverrides = @{
...
try {
    ...
    & python (Join-Path $RepoRoot "scripts\static_eq_external_holdout_smoke.py") `
        --output-dir $runDir --timeout-sec $TimeoutSec
    if ($LASTEXITCODE -ne 0) { Fail "External holdout smoke failed: $LASTEXITCODE" }
} catch { $runError = $_ }
finally {
    foreach ($key in $envOverrides.Keys) {
    ...
    foreach ($path in $protectedFiles) {
        $after[$path] = Get-HashOrEmpty $path
        if ([string]$before[$path] -ne [string]$after[$path]) { $changed += $path }
    }
    [ordered]@{
        schema_version = "static_eq.external_holdout_live_smoke_manifest.v1"
        timestamp = $stamp
        branch = (& git -C $RepoRoot branch --show-current)
        head = (& git -C $RepoRoot rev-parse HEAD)
        holdout_instance_count = 5
        protected_runtime_before = $before
        protected_runtime_after = $after
...
Write-Host "PASS: frozen external EQ holdout smoke" -ForegroundColor Green

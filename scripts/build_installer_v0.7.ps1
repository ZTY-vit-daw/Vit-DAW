...
<#
    One-click build for Vit DAW v0.7 Windows installer.
    ...
    Usage:
      powershell -ExecutionPolicy Bypass -File .\scripts\build_installer_v0.7.ps1
      powershell -ExecutionPolicy Bypass -File .\scripts\build_installer_v0.7.ps1 -SkipInnoCompile
#>
[CmdletBinding()]
param(
    [string]$UiExe = "D:\Vit_DAW\Export\Vit DAW v0.7.exe",
    [string]$ReleaseDir = "",
...
if ([string]::IsNullOrWhiteSpace($ReleaseDir)) {
    $ReleaseDir = Join-Path $RepoRoot "Export\_build\Vit_DAW_v0.7_release"
}
...
$IssPath = Join-Path $RepoRoot "Export\installer\Vit_DAW_setup_v0.7.iss"
...
Write-Host "[1/2] Assembling v0.7 layered release folder ..."
$assembleArgs = @{
    GodotUiExe         = $UiExe
    OutDir             = $ReleaseDir
...
$InstallerPath = Join-Path $RepoRoot "Export\installer\Vit_DAW_0.7_Setup.exe"

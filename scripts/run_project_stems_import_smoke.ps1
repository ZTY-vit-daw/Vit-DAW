...
param(
    ...
    [string]$SealedFolder = "E:\BaiduNetdiskDownload\yingge - Weekend Lover tracks out",
...
$resolvedSealedFolder = ""
if (-not [string]::IsNullOrWhiteSpace($SealedFolder) -and (Test-Path -LiteralPath $SealedFolder)) {
    $resolvedSealedFolder = (Resolve-Path -LiteralPath $SealedFolder).Path
}

$stamp = Get-Date -Format "yyyyMMdd_HHmmss"
$artifactDir = Join-Path $RepoRoot ("VitApp\Workspace\Artifacts\smoke\project_stems_import_" + $stamp)
$summaryPath = Join-Path $artifactDir "summary.json"
$probeOutput = Join-Path $artifactDir "project_stems_import_probe.json"
$probeLog = Join-Path $artifactDir "project_stems_import_probe.log"
$tempProjectDir = Join-Path $artifactDir "temp_project"
$tempProjectPath = Join-Path $tempProjectDir "project_stems_import.vit"
New-Item -ItemType Directory -Force -Path $artifactDir | Out-Null
New-Item -ItemType Directory -Force -Path $tempProjectDir | Out-Null

$summary = [ordered]@{
    schema_version = "project_stems_import_smoke_wrapper.v1"
    status = "running"
    started_at = (Get-Date).ToString("o")
    repo_root = $RepoRoot
    kernel_exe = $KernelExe
    training_folder = $TrainingFolder
    sealed_folder = $resolvedSealedFolder
...
try {
    ...
    Write-Step "Run project stems import probe"
    $probe = Join-Path $RepoRoot "scripts\project_stems_import_probe.py"
    $probeArgs = @(
        $probe,
        ...
        "--output", $probeOutput,
        "--req-timeout-ms", ([string]([Math]::Max(180000, $TimeoutSeconds * 1000)))
    )
    if (-not [string]::IsNullOrWhiteSpace($resolvedSealedFolder)) {
        $probeArgs += @("--sealed-folder", $resolvedSealedFolder)
    }
    & $PythonExe @probeArgs *>&1 | Tee-Object -FilePath $probeLog
    if ($LASTEXITCODE -ne 0) {
        Fail ("project_stems_import_probe.py failed with exit code " + $LASTEXITCODE + "; log=" + $probeLog)
    }
    $probeReport = Get-Content -LiteralPath $probeOutput -Raw | ConvertFrom-Json
    if ([string]$probeReport.status -ne "passed") {
        Fail ("project stems import probe failed: " + [string]$probeReport.error)
    }
    $summary["steps"] += @([ordered]@{
        name = "Run project stems import probe"
        status = "passed"
        log_path = $probeLog
    ...
    ConvertTo-JsonFile -Value $summary -Path $summaryPath
    Write-Ok ("project stems import smoke passed; summary=" + $summaryPath)
}
catch {

...
<#
    One-click build for Vit DAW v0.6 Windows installer (minimal payload: embed Python, kernel, bridge, Godot UI).
    Usage:
      powershell -ExecutionPolicy Bypass -File .\scripts\build_installer_v0.6.ps1
#>
[CmdletBinding()]
param(
    [string]$UiExe = "D:\Godot\project\vit-daw-frontend\Vit DAW v0.6.exe",
    [string]$ReleaseDir = "",
...
if ([string]::IsNullOrWhiteSpace($ReleaseDir)) {
    $ReleaseDir = Join-Path $RepoRoot "Export\_build\Vit_DAW_v0.6_release"
}
...
$IssPath = Join-Path $RepoRoot "Export\installer\Vit_DAW_setup_v0.6.iss"
...
$InstallerPath = Join-Path $RepoRoot "Export\installer\Vit_DAW_0.6_Setup.exe"

#!/usr/bin/env bash
# run_project_audio_settings_preflight_smoke mac equivalent — PORT-SMOKE-MAC-1
# (script 2/7).
#
# Mac port of scripts/run_project_audio_settings_preflight_smoke.ps1. Same
# two-step semantics — prepare a VitApp kernel, then drive the cross-platform
# probe scripts/project_audio_preflight_probe.py over kernel ZMQ and require
# its report status == passed:
#
#   | ps1 (run_project_audio_settings_preflight_  | mac (this script)          |
#   |        smoke.ps1)                            |                            |
#   |----------------------------------------------|-----------------------------|
#   | param($RepoRoot="D:\Vit_DAW", $KernelExe,    | flags below (same names)    |
#   |   $TrainingFolder="E:\…yingge - sattelites   |                             |
#   |   tracks out", $PythonExe, $ZmqReqPort,      |                             |
#   |   -ReuseKernel, -KeepProcess, $TimeoutSeconds=45)                          |
...
#   |   Artifacts\smoke\project_audio_preflight_*  |   dir (default              |
#   |                                              |   ~/Documents/vit-smoke-    |
#   |                                              |   mac1-artifacts/<run_id>)  |
#   | summary.json wrapper schema                  | same fields, suffix .mac.v1 |
#   |   project_audio_settings_preflight_          |   + run meta (HEAD+dirty)   |
#   |   smoke_wrapper.v1                           |                             |
...
usage() {
  cat <<'EOF'
run_project_audio_settings_preflight_smoke_mac.sh — mac port of
run_project_audio_settings_preflight_smoke.ps1 (PORT-SMOKE-MAC-1).
...
Options:
  ...
  --timeout-seconds N     Startup/REQ timeouts (default 45)
...
RUN_ID="project_audio_preflight_mac_$(date '+%Y%m%d-%H%M%S')"
...
TEMP_PROJECT_PATH="$WORKDIR/temp_project/project_audio_settings_roundtrip.vit"
PROBE_OUTPUT="$WORKDIR/project_audio_preflight_probe.json"
PROBE_LOG="$WORKDIR/project_audio_preflight_probe.log"
...
[[ -f "$REPO_ROOT/scripts/project_audio_preflight_probe.py" ]] || fail_env "probe script missing: $REPO_ROOT/scripts/project_audio_preflight_probe.py"
...
write_summary() {
  ...
  python3 - "$SUMMARY_PATH" "$RUN_ID" "$WORKDIR" "$status" "$error" "$ended_at" \
    ...
    "$KERNEL_STOP_RECORD" "$KERNEL_PID" "$REPO_ROOT" <<'PY'
import json, os, sys
(out, run_id, workdir, status, error, ended_at, kernel_bin, training_folder,
 project_path, probe_output, kernel_stop, kernel_pid, repo_root) = sys.argv[1:14]
steps = []
probe_meta = {}
if os.path.isfile(probe_output):
    try:
        probe = json.load(open(probe_output, encoding="utf-8"))
        steps.append({"name": "Run project audio settings/preflight probe",
...
summary = {
    "schema_version": "project_audio_settings_preflight_smoke_wrapper.mac.v1",
    "card": "PORT-SMOKE-MAC-1",
    "ps1_source": "scripts/run_project_audio_settings_preflight_smoke.ps1",
    "run_id": run_id,
    "status": status,
    "error": error,
...
# ---------------------------------------------------------------- probe
step "Run project audio settings/preflight probe"
REQ_TIMEOUT_MS=$(( TIMEOUT_SECONDS * 1000 ))
(( REQ_TIMEOUT_MS >= 30000 )) || REQ_TIMEOUT_MS=30000
set +e
"$PYTHON_BIN" "$REPO_ROOT/scripts/project_audio_preflight_probe.py" \
...
set -e
[[ "$PROBE_EXIT" -eq 0 ]] \
  || { write_summary failed "project_audio_preflight_probe.py failed with exit code $PROBE_EXIT; log=$PROBE_LOG"; \
       fail_functional "project_audio_preflight_probe.py failed with exit code $PROBE_EXIT; log=$PROBE_LOG"; }
...
if [[ "$PROBE_STATUS" != "passed" ]]; then
  ...
  write_summary failed "project audio preflight probe failed: $PROBE_ERROR"
  fail_functional "project audio preflight probe failed: $PROBE_ERROR"
fi

write_summary passed ""
ok "project audio settings/preflight smoke passed; summary=$SUMMARY_PATH"
log "workdir: $WORKDIR"
exit 0

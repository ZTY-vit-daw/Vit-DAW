...
$runDir = Join-Path $RepoRoot ("VitApp\Workspace\Artifacts\smoke\de_esser_control\" + $stamp)
...
try {
    ...
    $args = @((Join-Path $RepoRoot "scripts\de_esser_matrix_smoke.py"),
    ...
    & python @args
    if ($LASTEXITCODE -ne 0) { throw "De-esser $Phase failed" }
}
finally {

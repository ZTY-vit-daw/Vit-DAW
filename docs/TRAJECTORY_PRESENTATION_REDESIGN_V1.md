...
### B 澄清（2026-09-14 下午，用户质询后补记）

用户对照 ZCode/DSH 使用经验质询：成熟 agent 长轮次任务**无用户面「继续」需求**，停等只出现在交互卡片（选择/权限/判定）。成立。据此：本稿用户面停等的长期对齐目标=**只出现在交互卡上**（A/B 判定卡/选择/权限）+终局+用户主动停止；「等待续跑」显示行是过渡期的诚实显形（文案按等待对象区分：有未决 A/B 卡→「等待你的试听判定」），行为面根治（欠收尾评估自动续跑）另立 TRAJ-AUTO-SETTLE-1。

...
1. `~/.vit/free_state_experiment_plugins.json` 的注入条目必须通过 PCA promoted + 当前二进制指纹校验。**注意 static_eq 是 V1 家族**（processorattestation/types.go:18；V1 专属查询 store.go:248-258，bx_hybrid V2 的 promoted 认证实测在 v1 库 `processor_control_attestations.v1.json`）——正确谓词是 **V1** `processorattestation.QueryLibraryAdmission(libraryV1, subjectKey, fingerprint, FamilyStaticEQ)`（v1 过滤模式 plugin_recommendation_attestation.go:226-262：BuildSubjectKey + FingerprintPath + Query）；用 QueryLibraryAdmissionV2 传 static_eq 会报 "unsupported processor family"（IsV2Family 不含 static_eq，v2.go:89-92）。不 Eligible 则拒绝执行并报可区分错误。

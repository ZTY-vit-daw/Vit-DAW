...
- B2 now passes post-execution acoustic verification only when the observation is fresh, `level_distribution` is `ready`, compared-track coverage is at least 95%, and every action target remains in the fresh MOM inventory. Missing band/stereo evidence remains an optional limitation.

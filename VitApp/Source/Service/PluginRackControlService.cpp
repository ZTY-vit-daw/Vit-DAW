...
juce::String parameterSurfaceSignature (const juce::Array<juce::var>& parameters)
{
    ...
    for (const auto* byte = reinterpret_cast<const unsigned char*> (joined.toRawUTF8()); *byte != 0; ++byte)
    {
        hash ^= static_cast<unsigned long long> (*byte);
        hash *= prime;
    }
    return "probe_" + juce::String::toHexString (static_cast<juce::int64> (hash));
}
...
juce::String PluginRackControlService::handleGetPluginParameters (const juce::DynamicObject& object, const juce::String&) const
{
    ...
    const auto currentSignatureHash = parameterSurfaceSignature (parametersArray);

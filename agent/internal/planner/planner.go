...
- For runtime acoustic plugin adjustments on an already loaded/learned plugin, such as cutting mud near 500Hz, boosting presence, or reducing harshness, use plugin_grabber.apply_control with control eq.cut_region/eq.boost_region/eq.set_region and target freq_hz/gain_db/q when known. Do not use raw plug-in parameter mutation; use the governed semantic B4 path.
- Raw plug-in parameter mutation is not model-visible. Plan semantic controls through the governed B4 plug-in effect path, and fail closed when no verified runtime profile can resolve the requested control.

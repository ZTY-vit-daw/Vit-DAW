// Package pluginprobe exposes an isolated, observation-only VST3 host.
package pluginprobe
...
const VST3HostAdapterProtocol = "vit.pluginprobe.vst3_host_adapter.v1"

// VST3HostAdapter owns a single crash-isolated native worker. It can load an
// isolated instance, observe its parameter surface, and render probe audio. It
// has no parameter-write, profile, mapping, or Vit project authority.
...
// Handler exposes a stable, loopback-only observation API. Binding to a
// loopback address is enforced by the CLI.
func (a *VST3HostAdapter) Handler() http.Handler {
	...
	mux.HandleFunc("/health", func(w http.ResponseWriter, r *http.Request) {
		...
		writeHTTP(w, http.StatusOK, map[string]any{"status": "ok", "service": "pluginprobe-vst3-host-adapter", "authority": "observation_only", "state": a.state()})
	})
...
// NormalizeVST3WorkerSnapshot turns raw native observations into the stable
// pluginprobe snapshot while retaining the raw native response as evidence.
// It performs no semantic inference or mapping action.
func NormalizeVST3WorkerSnapshot(raw json.RawMessage, logs []string) (HostSnapshot, error) {
	...
	for _, parameter := range source.Parameters {
		...
		if parameter.Boolean {
			...
			// Keep observed bounded discrete parameters aligned with the
			// parameter bridge's canonical enum fingerprint grammar.
			kind = "enum"
		}

...
func TestAudioClosureNoCandidateReplyCarriesTrackContext(t *testing.T) {
	...
	reply := audioClosureSettlementReply(singleTrack, 1, freeStateCandidateSupply{})
	...
	multiTrack := audioClosureSettlementReply(&audioclosure.Settlement{Reason: audioclosure.StopNoCandidateFound}, 6, freeStateCandidateSupply{})
	...
	unknown := audioClosureSettlementReply(&audioclosure.Settlement{Reason: audioclosure.StopNoCandidateFound}, -1, freeStateCandidateSupply{})
	if strings.Contains(unknown, "只有 1 轨") {
		t.Fatalf("unknown track count must fall back to generic wording: %q", unknown)
	}

	other := audioClosureSettlementReply(&audioclosure.Settlement{Reason: audioclosure.StopDiagnosticComplete}, 1, freeStateCandidateSupply{Trimmed: true, Delivered: true})

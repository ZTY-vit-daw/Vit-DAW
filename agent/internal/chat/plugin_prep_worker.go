...
func (s *Server) pluginPrepWorkerFromDigest(plan PendingPlan, trackID, pluginID, pluginName string, params map[string]any, message string, baseEvents []map[string]any) pluginPrepContinuation {
	...
	candidate := agentprotocol.PendingCandidate{
		...
		TargetRef:       targetRef,
		ActionKind:      cleanContextText(action["action_kind"]),
		ProcessorType:   cleanContextText(action["processor_type"]),
		EvidenceRefs:    pluginPrepWorkerStringList(action["evidence_refs"]),
		NeedsResolution: pluginPrepWorkerStringList(action["needs_resolution"]),
		Confidence:      cleanContextText(action["confidence"]),

...
func BuildStaticBalancePack(in StaticBalanceInput) StaticBalancePack {
	...
	model := staticbalance.BuildModel(staticbalance.Input{
		...
		AudioAnalysisStatus: in.AudioAnalysisStatus, TOMProjection: in.TOMProjection,
	...
	return StaticBalancePack{
		...
		Scope:             Scope{Kind: "project", Source: "project.state+DAD+TOM+MOM"},
		Budget:            budget,
		...
		EvidenceStatus: map[string]EvidenceStatus{
			...
			"dad_levels":    {Status: staticBalanceStatus(model.Coverage.DADLevelKnownCount > 0), KnownCount: model.Coverage.DADLevelKnownCount, TotalCount: len(model.Tracks)},

...
- For an explicit plugin effect control request (EQ, compression, or any other effect), use this two-tier approach. TIER 1 — verified profile only: use plugin_grabber.apply_control ONLY when runtime_profile.virtual_controls is non-empty AND eq_band_summary is absent from the explain_controls result. TIER 2 — when eq_band_summary is present OR no virtual_controls exist: the result of plugin_grabber.explain_controls already contains eq_band_summary with exact param_ids, domain info, and how_to_pick_a_band — do NOT call apply_control at all, go directly to plugin.set_parameter. Follow the how_to_pick_a_band instruction in the summary exactly: it specifies the correct write sequence for the EQ type (fixed_slot_adjustable writes both freq and gain of the closest band; fixed_freq writes only gain; free_floating creates a new band). After writing, call get_plugin_parameters to confirm. Never pass value_text.

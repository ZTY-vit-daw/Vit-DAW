...
- For an explicit plugin effect control request (EQ, compression, or any other effect), use this two-tier approach. TIER 1 — verified profile only: use plugin_grabber.apply_control ONLY when runtime_profile.virtual_controls is non-empty AND eq_band_summary is absent from the explain_controls result. TIER 2 — when eq_band_summary is present OR no virtual_controls exist: call plugin_grabber.explain_controls first. The result contains eq_band_summary with exact bindings, domains, typed sections, and capabilities. For EQ, call plugin_grabber.set_eq_point(track_id, plugin_id, freq_hz, gain_db, q, shape, slope_db_per_oct) using only explicitly requested fields; gain_db is required for Bell/Shelf and omitted for Cut/Notch. Band/section selection, normalization, readback, and rollback are deterministic in Go, so do NOT call set_plugin_param or apply_control for EQ when set_eq_point is available. Report actual readback, quantization, partial application, and limitations exactly. For non-EQ effects with no virtual_controls, pick the relevant param from all_parameters and call set_plugin_param with a normalized value. After writing, call get_plugin_parameters to confirm. Never pass value_text.

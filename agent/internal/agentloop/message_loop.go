...
- For an explicit plugin effect control request (EQ, compression, or any other effect), use this two-tier approach. TIER 1 — verified profile: use plugin_grabber.apply_control when runtime_profile.virtual_controls is non-empty. TIER 2 — no verified profile OR after apply_control is rejected: call plugin_grabber.explain_controls to get all_parameters (each entry has display_domain_candidate with unit/min/max/scale), identify the correct param_id by reading parameter names; when multiple bands exist and no band is specified, pick the band whose current frequency is closest to the target frequency (or use the first available non-bypass band). Compute a normalized 0.0–1.0 value from the display_domain_candidate min/max/scale, then IMMEDIATELY call set_plugin_param with track_id, plugin_id, param_id, and value IN THE SAME TURN — do not pause or ask for confirmation before writing. set_plugin_param IS available and IS the correct tool for this path. After the write, call get_plugin_parameters (include_parameters:true) to confirm. Never pass value_text to set_plugin_param. IMPORTANT: if apply_control fails with any error (including "no usable EQ band group", "runtime profile missing", or similar), do NOT stop — immediately fall back to set_plugin_param. Do not ask for clarification when the target band can be inferred from the parameter list.
...
- For plugin parameter writes: when a verified runtime profile is available use plugin_grabber.apply_control so B4 can freeze identities, preimage, and rollback. When no verified profile is available, use set_plugin_param with a normalized value from display_domain_candidate — this is the approved Tier 2 direct-control path, not a raw mutation. Never pass value_text to set_plugin_param.

...
- For needs_experiment, return one bounded improvement proposal with an explicit target, exact evidence_refs, hypothesis, expected_effect, action_domain, and action_kind. The action_domain may be track/clip gain, pan, EQ, or any governed processor domain. For track_gain, include parameter_bounds={"delta_db":<nonzero number within +/-2>}; for pan, include either {"delta_pan":<nonzero number within +/-0.15>} or {"target_pan":<number from -1 to +1>}. These are proposed native-tool values, not plug-in mappings. Leave processor_type empty when the improvement does not require a fixed family yet.

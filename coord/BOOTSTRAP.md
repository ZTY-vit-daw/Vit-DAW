...
- 定时/心跳自动化已移除（用户裁定 2026-09-16）；watcher 只在用户激活的值班流内运行（流关闭即停，启用授权见 PROTOCOL §4）
...
- 先报告再动手：仓库/远端变更不构成行动授权；值班流内的 watcher 自运维与只读命令除外
- watcher 启用须经用户确认：只在用户激活的值班流内启动，双端不得自行启用，跨端不得代启（SSH 仅限只读核查，2026-09-16 裁定）

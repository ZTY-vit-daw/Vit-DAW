...
**定时任务已移除（用户裁定 2026-09-16 晚）**：双端不再保留任何定时/心跳自动化。watcher 死亡（流关闭、进程退出、假触发后未重启）由用户下次在该端值班流中的交互或新开「值班」口令自愈；空窗期内远端事件不被感知是本模式接受的代价。

**watcher 启用授权（用户裁定 2026-09-16 晚，见 [decisions/2026-09-16-watcher-user-confirm.md](decisions/2026-09-16-watcher-user-confirm.md)）**：watcher 只能在**用户激活的值班流**内启动（「值班」口令或用户粘贴的备用开流提示词等价激活）；任何会话不得在值班上下文之外自行启动 watcher，跨端不得代启对端 watcher（SSH 面仅限只读状态核查）。值班流内唤醒会话处理完毕后的重启属该次授权内的自运维；流关闭 watcher 即停，不跨流存活。watcher 唤醒只产生汇报，任务执行授权仍只来自用户口令或流内批准（report-first 不变）。

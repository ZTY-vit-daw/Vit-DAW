...
- 回执（终稿 2026-09-21 22:10）：**三族全部定责完毕，产出三张修复卡（todo/）**：
  - F2=呈现路径丢话（模型零责实证：问句原文在 durable 存储找到）→ FIX-F2-SURFACE-REPLY；
  - F5=快照缺新鲜度标注+双写非原子（用户裁定前台铃机制：写入层自写标注）→ FIX-F5-SNAPSHOT-FRESHNESS；
  - F3=G4 与 §6 认知政策规则冲突+锁死轮出口陷阱（两次 gap 均仅 G4；门比脊柱 FS5 守卫更严不对齐；反向激励实证）→ FIX-F3-G4-SEMANTICS（用户裁定修 G4；方案乙推荐先行+方案甲待产品裁定）。
  - 补充发现：fallback 措辞 terminal_turn_unparseable 失实（本案为 gate-rejected）；F2 轮若呈现正确应为绿——修复会话 ⑤ 三红无一为模型推理问题。
- 验收：**pass（2026-09-21 用户会话验收：三问三答+三修复卡方向逐项确认，F5 采集逻辑修正与 F3 修 G4 由用户当场裁定）**
  ...
  - **F2（破案 2026-09-21 21:55）**：durable 对话存储（AppData/Local/Vit/ProjectHistory/drafts/draft_20260921T121201_d1624fc1/.vit_history/.sessions/…/agent_runtime_state.json）中 goal_continuations/goal_b702b714ecd21534 的 trace 与 context_snapshot 均含模型最终问句原文：**"需要先确认哪条是主唱轨。请告诉我主唱是 Track 几，或把主唱轨重命名为 vocal / 主唱后让我重新观察。"**——stop=needs_clarification 正确、问句内容完全满足断言分支(a)。**定责：模型 0 责；缺陷在呈现路径**——最终问句落在 continuation trace 而 settle 合成抓到的是工具步标题（turn 级事件 body 未承载最终回复或合成取行错误）。修复范围：turn 级事件应承载最终回复，或合成回退读 runtime status 的澄清文本。该轮若呈现正确应为绿。
  - **F3-Q2 精确化**：两次拒绝（20:18:20 / 20:19:53）的 gap 均**只含 G4_dimension_closed / no_closed_diagnostic_dimension 一条**，无其他门、无解析失败。
  - **F5 方向修正（用户质询驱动）**：永久性单侧不更新场景真实存在（先例：投影清单 §9 的 masking unavailable / mixed-tap 候选 withheld，当时按"显式 missing+理由"处理）。修复=写侧原子发布（消瞬态）+**读侧新鲜度标注**（旧行相对 latest_request 标 stale/superseded 而非裸 ready——正是 CCB"stale 原样保留不升级 readiness"纪律）；采集侧可加一次有界重读区分瞬态/永久，断言改为"不存在未标注的分叉"。

...
- 验收：**pass（2026-09-27 决策会话·mac，报告合入 f353438）**——四线全达标（18 view≥10/基线=尺度零参数化+选择面 5 参数/缺口 7≥6/入库）；决策侧抽核四发全中（freeStateViewDefinitions :421 定义点+18 view 计数 grep 复核=1 comparison+3 mix+3 processor+2 project+9 track、GetViewsForDimension 无生产调用 grep 亲验仅测试文件引用、masking candidates ≤12 :937-943 亲读、catalog.go 请求 schema 无 freshness 枚举亲读）；两层独立裁剪（capabilitycontext 白名单+contextruntime 分层预算）与两套键命名法发现采信为 L1-3/G1 输入；GetViewsForDimension 未接线发现入 L1-3 即席查询预备。

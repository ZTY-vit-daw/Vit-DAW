...
- 验收：**pass（2026-09-27 决策会话·mac，报告合入 bc300a2）**——五线全达标（6≥4 机制面五要素/三要素矩阵逐格/固化分级+PCA 唯一终身样板/惰性健康+三处冲突再确权+无批发式/入库）；决策侧抽核三发命中（PendingCandidate.EvidenceRefs agentprotocol/types.go:231、auditionJudgmentRequest :304-314 含 preference/supersedes_id/evidence_id、认证 token 一次性消费 processor_certification_entry.go:104-107）；**卡面回执锚点笔误勘正**：ApprovalRequest 实位于 agentprotocol/types.go:244-256（非 typed_protocol.go），"无 EvidenceRefs 字段"论断亲验成立（字段清单逐列核对）——笔误不影响实质；M5 唯一三要素齐全+M4 唯一持久确权样板=D13 统一协议起点结论采信，段落命名/角色身份两域零覆盖=净新增面入 D13 设计输入。

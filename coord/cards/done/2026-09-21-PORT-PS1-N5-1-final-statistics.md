...
- 回执：2026-09-21 11:05 / PC 执行侧（GLM-5.3） / 分支 port/ps1-n5-1 已推（59ccc8e1，SMOKE_TESTS.md 统计口径两行——卡面"如需补一行"预期内文档补充，非缺陷修复）；coord 领取 7b8bb1cc 已推 main。**结果：④ 3/5 ✗、⑤ 3/5 ✗——两件均不达标（<4/5），逐轮族谱如下；但 vocal focus 提案面停车率 4/4（PC 历史口径 7/7 → 11/11），放大器假说标的在 PC 侧保持 100%，⑤ 的红轮均非 vocal focus 停车失败**。
  ...
  - **【修复后复测注记 2026-09-23，决策侧据 mac 跟随回执拼入】** mac 在五补丁+脚本移植树（5678a79+bbd0dbf，flash+白名单，驱动镜像 WL-1）复测 ④⑤×5 对照 WL-1 列：**④ 2/5→4/4**（R4 环境批另计：起栈 chat 传输失败——F2×2/F4×1 红轮族全吸收）；**⑤ 3/5 持平但族迁移**（F3/F5复合清零；新形态 clarify 链失败×1〔FORENSIC-MAC-CLARIFY-CHAIN-1 取证中〕+F4×1）；停车率 4/5→3/5。补丁实效直接证据：三个停车轮 reply 均天然含问句（Go 层 F2 直供，脚本兜底零替换）。数字详证=[2026-09-23-mac-followup-f5f2-receipt.md](../../reports/2026-09-23-mac-followup-f5f2-receipt.md)——本表三列保持修复前基线不动，修复后列供论文另行成表。**夹具口径（2026-09-23 补注，用户质询驱动核实）**：④⑤ 双端均用仓内合成夹具（test_100hz_10s.wav=100Hz 正弦+test_target_3s.wav=3s 目标音，tracked 双端同源）——本表数字证明**工作流机制**（停车/双跳/settle/准入/门语义），不构成提案内容质量证据（合成音无频段占用/动态问题，插件族提案无素材动机、模型此类建议无工程依据）；内容质量证据层=真实素材旅程/AB 烟测（素材 PC 本机不入仓，mac 侧由 PORT-REALSTEMS-MAC-1 补齐）。论文/答辩引用本表数字时须带此口径。

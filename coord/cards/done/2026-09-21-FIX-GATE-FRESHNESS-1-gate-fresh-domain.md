...
- 验收：**pass（2026-09-22 决策会话，用户 ⑤ 指示直接认定路径）**——验收①②按回执采信（红测试三场景 R4 逐字一致复现+真拒绝保留+预算保护；agent 全量+webui 绿；夹具两处更新=卡面目标行为，复核认可——旧"空选择即拒"夹具正是 R4 滞后窗形态）。验收③之 ④ 1/1 exit 0 采信；⑤ exit-0 由替代认定**通过**：F2/F5/F3 修复后 ⑤ 已三次整轮 exit 0（20260921_220450 F5 / 20260921_221052 F2 / 20260922_183310 F3 run3），本卡 ⑤ 三红均属三张修复卡域内族、与 G 门读路径无交集（回执证据+FORENSIC-45 定责在案）。三项上交裁定：a) **⑤ exit-0 替代认定=成立**（如上）；b) **clarify-ask goal=failed 独立取证卡=销项**——FORENSIC-45-FAMILIES 已完成该族定责（G4 语义+锁死轮出口陷阱，产出 FIX-F3-G4-SEMANTICS 修复并验收通过），不再另开取证卡；c) **mac 跟随=随 2026-09-22 转交包走**（coord/reports/2026-09-22-mac-followup-package.md，同一 diff 双端同步语义，无词表变更）。**8653a25a 的 main 合入由 FIX-FRONTIER-FOLD-1 item1 执行（冲突重放后两侧测试组共存全绿为该卡验收口径），合并后 ⑤ 1 轮复核为附加验证、不阻塞本卡验收**。

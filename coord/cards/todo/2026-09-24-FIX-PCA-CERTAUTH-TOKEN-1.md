# FIX-PCA-CERTAUTH-TOKEN-1：static_eq 认证授权入口补齐——解锁 57 例 load gate 循环依赖

- 优先级 / 预估 / 依赖：P2 / 0.3 天 / FIX-PCA-EQCHANNEL-1 验收上交裁定（2026-09-24）；sweep 幂等续跑就绪（retry 语义），解锁即自动吸收
- 模型分级：L1-L2 / GLM-5.3（授权面改动，红测试先行；~40-60 行）
- **背景（EQCHANNEL 取证）**：harness 认证授权白名单已含 static_eq（harness.go:1956）但**无入口可铸授权**——runner 入口 processor_certification_entry.go:419 拒 static_eq；full-access 路要求先晋升=循环依赖；HTTP invoke 强制 source=http（server.go:1463）。57 例 static_eq 主体卡在 load_gate_blocked（56 无晋升+1 跨族歧义）。
- 目标：

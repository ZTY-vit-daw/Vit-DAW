...
juce::File defaultProbeAudioFile()
{
    ...
    const auto bundled = root.getChildFile ("assets")
                             .getChildFile ("vps_probe_audio_v1")
...
// network listener and does not write a VPS workspace, Credential, Catalog or
// SPAL route. The host bypass below is a direct source-to-output A/B switch;
...
public:
    explicit LiveMonitor (juce::AudioProcessor& processor)
        : plugin (processor), readAheadThread ("VPS Forge witness source reader")
    {
...
private:
    ...
    void showAudioSettings()
    {
        ...
        options.dialogTitle = "VPS Forge Witness Audio Device";
...
private:
    ...
    static LaunchOptions parseOptions (const juce::StringArray& args)
    {
        ...
        for (int index = 0; index < args.size(); ++index)
        {
            ...
            else if (argument == "--audio-file" || argument == "--source")
            {
                ...
                parsed.error = "Usage: vpsforge_vst3_witness.exe --plugin-path <plugin.vst3> [--audio-file <source.wav>] [--no-loop] [--input-gain-db -18] [--sample-rate 48000] [--block-size 512]";
    ...
    void openWitness()
    {
        ...
        const auto title = "VPS Forge Witness - " + descriptions.getFirst()->name + " (live, isolated)";
